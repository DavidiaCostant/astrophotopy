...
class Image:
    ...
    def histogram(self):
        ...
        self.norm = np.dot(np.transpose(self.hist),self.hist)/len(self.hist) #norm of the image (intended as a vector) normalized on pixel number
    ...
    def blur(self, kernel):
        self.image = cv2.blur(self.image, (kernel, kernel))
...
class FlexIm:
    ...
    def find_rappresentative(self):#find the rappresentative image within a set (on st.dev. basis)
        best_k = 10
        index = 0
        for item in self.container:
            if item.dev < best_k:
...
class Telescope:
    def __init__(self, **kwargs):
        ...
        print("Telescope "+self.model+" created\n")
...
class Project:
    ...
    def calc_super_res_number(self):
        #utility method for calculation of the minimal set dimension for "bayesian multi-image upscaling" approach
        magnification = self.telescope['f_len']*self.telescope['lens']/self.telescope['f_len_ep']
        resolution = (np.sqrt(2)*self.telescope['sens_diag']*1000/(self.telescope['px_dim']))*np.arctan(0.5*self.telescope['sens_diag']/self.telescope['f_len'])
        return round((self.telescope['fwhm']*magnification/resolution)**2)
